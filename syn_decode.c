...
void encode(uint8_t** codeword, int64_t prime, int64_t n, int64_t redundancy, int64_t packet_size) {
    ...
    syndrome_based_decode(codeword, prime, n, redundancy, packet_size, erasureIndex, redundancy);
    free(erasureIndex);
    erasureIndex = NULL;
}
...
int main() {
    ...
    while (choice) {
        ...
        for (int s = 0; s < stripes; ++s) {
            for (int64_t i = 0; i < total; ++i) {
                syndrome_based_decode(codeword, p, n, r, packet_size, patterns[i], lambda);
                if (checksum(codeword, p, n, r, packet_size)) {
